...
# convert date columns into datetime object (confirming with .dtypes)

df_both_small[['user_join_date','collected_at', 'created_at']] = df_biden[['user_join_date','collected_at', 'created_at']].apply(pd.to_datetime, format='%Y-%m-%d %H:%M:%S.%f')
df_both_small.dtypes

# create column in year-month-day format

df_both_small['day_tweeted'] = df_both_small['created_at'].dt.strftime('%Y-%m-%d %H' + ":00")

hourly_tweeting = df_both_small.groupby(['day_tweeted']).size().reset_index(name='counts')
...
# df_biden[['user_join_date','collected_at', 'created_at']] = df_biden[['user_join_date','collected_at', 'created_at']].apply(pd.to_datetime, format='%Y-%m-%d %H:%M:%S.%f')
# df_donald[['user_join_date','collected_at', 'created_at']] = df_donald[['user_join_date','collected_at', 'created_at']].apply(pd.to_datetime, format='%Y-%m-%d %H:%M:%S.%f')